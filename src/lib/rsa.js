...
RSA.ajax = function(options,getkeysurl){
	var encryptedOptions = options;
    var data = options.data;
    var me = this;
    var value = JSON.stringify(data);
    $.jCryption.getKeys(getkeysurl,function(keys){

		 var bt = strToBt(value);
		 var seed = Math.random();
		 var hashObj = new jsSHA(seed.toString(), "ASCII");
		 var keyDES = hashObj.getHash("SHA-512", "HEX");
		 var enchex = strEnc(value,keyDES);
		     
        $.jCryption.encryptKey(keyDES, keys, function(encryptedString){
        	encryptedOptions.data = {};
        	encryptedOptions.data.value=enchex;
        	encryptedOptions.data.key=encryptedString;
        	encryptedOptions.headers={"encrypted":"encrypted"};
        	$.ajax(encryptedOptions);
        });
    });
...
RSA.desAjax = function(options){
    ...
    var exceptionurl = data.url;
    
    encryptedOptions.headers={"encrypted":"encrypted"};

    var decryptedSuccess = function(data, textStatus, jqXHR){
    	var decryptedKey = "864c60d";
    	var decryptedData ;
    	var obj;
    	//
    	if(exceptionurl.indexOf("/data/order/getAvPrice.do")>-1 || exceptionurl.indexOf("/data/order/getAvPriceForJson.do")>-1){
    		...
    		decryptedData = strDec(data.data,decryptedKey);
...
//        	var obj = JSON.parse(decryptedData);
    	success.call(this,obj,textStatus,jqXHR);
    }

    encryptedOptions.success = decryptedSuccess;

    $.ajax(encryptedOptions);

}
RSA.desStr = function(param) {
	var keyDES = "0979ebb";
	var enchex = strEnc(param,keyDES);
	return enchex;
}
...
function strDec(data,firstKey,secondKey,thirdKey){
 ...
 for( let i = 0;i < iterator;i++){
   ...
   if(firstKey != null && firstKey !="" && secondKey != null && secondKey != "" && thirdKey != null && thirdKey != ""){
     ...
     var x,y,z;
     tempBt = intByte;
     for( let x = thirdLength - 1;x >= 0;x --){
       tempBt = dec(tempBt,thirdKeyBt[x]);
     }
     for( let y = secondLength - 1;y >= 0;y --){
       tempBt = dec(tempBt,secondKeyBt[y]);
     }
     for( let z = firstLength - 1;z >= 0 ;z --){
     ...
     if(firstKey != null && firstKey !="" && secondKey != null && secondKey != ""){
       ...
       var x,y,z;
       tempBt = intByte;
       for( let x = secondLength - 1;x >= 0 ;x --){
         tempBt = dec(tempBt,secondKeyBt[x]);
       }
       for( let y = firstLength - 1;y >= 0 ;y --){
